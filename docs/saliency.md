...
4. Wilcoxon paired significance tests (script + CSV output)
5. Grad-CAM vs Grad-CAM++ delta diagnostics
6. Qualitative Grad-CAM / Grad-CAM++ side-by-side panel generation
...
## Wilcoxon Paired Significance Tests (Primary Slice)

This is run after Stage-7 as a significance check run via script (not generated by the notebook) in `scripts/compute_wilcoxon_stats.py`.

Please refer to the README for instructions for running in the CLI Flow section.

### Wilcoxon Test Specification

| Item | Value |
| --- | --- |
| Statistical test | Paired Wilcoxon signed-rank (`scipy.stats.wilcoxon`) |
| Input metric source | Per-image insertion AUC from `per_image_scores.csv` |
| Input path used by script | `artifacts/metrics/faithfulness/faithfulness/per_image_scores.csv` |
| Filter | `method=gradcam`, `slice=primary` |
| Pairing strategy | Match by common `test_image_id` per seed and condition pair |
| Seeds | `0, 1, 2` |
| Condition pairs tested | `supervised-moco`, `supervised-swav`, `supervised-random_init`, `swav-moco`, `random_init-moco`, `swav-random_init` |
| Output file | `artifacts/metrics/faithfulness/wilcoxon_paired_stats_primary.csv` |

### Result Summary

From `artifacts/metrics/faithfulness/wilcoxon_paired_stats_primary.csv`:

| Comparison | p (seed 0) | p (seed 1) | p (seed 2) | Significant at 0.05 |
| --- | --- | --- | --- | --- |
| `supervised` vs `moco` | `0.0` | `0.0` | `0.0` | `3/3 seeds` |
| `supervised` vs `swav` | `0.0` | `0.0` | `0.0` | `3/3 seeds` |
| `supervised` vs `random_init` | `0.0` | `0.0` | `0.0` | `3/3 seeds` |
| `swav` vs `moco` | `0.0` | `0.0` | `0.0` | `3/3 seeds` |
| `random_init` vs `moco` | `0.0` | `0.0` | `0.0` | `3/3 seeds` |
| `swav` vs `random_init` | `0.025872` | `0.098352` | `0.410603` | `1/3 seeds` |

Interpretation:

- The Wilcoxon results support strong pairwise separation for most condition comparisons.
- `swav` vs `random_init` is not consistently significant across seeds, indicating weaker separation under this test.
- This test complements the paired bootstrap/permutation analysis reported above.

## Grad-CAM++ Diagnostics and Interpretations
...
Below are the outputs written by the notebook and companion scripts:
...
| Wilcoxon paired stats | `artifacts/metrics/faithfulness/wilcoxon_paired_stats_primary.csv` |
